...
// For the hide and show arrows

let arrow = document.getElementsByClassName("first-arrow");

let secondArrow = document.getElementById("reportShow").innerHTML;

let firstArrow1 = arrow[3].innerHTML;

let firstArrow2 = arrow[2].innerHTML;

let firstArrow3 = arrow[1].innerHTML;

let firstArrow4 = arrow[0].innerHTML;

function general() {
  let doc = document.getElementById("general-content");

  const disp = getComputedStyle(doc).getPropertyValue("display");

  if (disp == "block") {
    doc.style.display = "none";
    arrow[0].innerHTML = firstArrow4;
  } else {
...
function subtitle() {
  let doc = document.getElementById("subtitle-content");

  const disp = getComputedStyle(doc).getPropertyValue("display");

  if (disp == "block") {
    doc.style.display = "none";
    arrow[3].innerHTML = firstArrow1;
  } else {
    arrow[3].innerHTML = secondArrow;
    doc.style.display = "block";

    document.getElementById("sub-text").scrollIntoView();
  }
}

function audioSet() {
  let doc = document.getElementById("audioSet-content");

  const disp = getComputedStyle(doc).getPropertyValue("display");

  if (disp == "block") {
    doc.style.display = "none";
    arrow[2].innerHTML = firstArrow2;
  } else {
    arrow[2].innerHTML = secondArrow;
    doc.style.display = "block";

    document.getElementById("audio-click").scrollIntoView();
  }
}
...
function player() {
  let doc = document.getElementById("player-content");

  const disp = getComputedStyle(doc).getPropertyValue("display");

  if (disp == "block") {
    doc.style.display = "none";
    arrow[1].innerHTML = firstArrow3;
    showScreenCount = 0;
  } else {
    arrow[1].innerHTML = secondArrow;
    doc.style.display = "block";
    document.getElementById("play-text").scrollIntoView();
  }
}
...
(() => {
  document.getElementById("brightValue").innerHTML = `${
    document.getElementById("brightness").value
  }`;

  let range = document.getElementById("brightness");

  let upper = Number((range.value / 10) * 100);

  let lower = Number(((range.value - 10) / 10) * 100);

  range.style.background =
    `linear-gradient(to right, indigo ${upper}%, slategrey ${lower}%)` ||
    `-o-linear-gradient(to right, indigo ${upper}%, slategrey ${lower}%)` ||
    `-ms-linear-gradient(to right, indigo ${upper}%, slategrey ${lower}%)` ||
    `-moz-linear-gradient(to right, indigo ${upper}%, slategrey ${lower}%)` ||
    `-webkit-linear-gradient(to right, indigo ${upper}%, slategrey ${lower}%)`;
})();
...
function bright() {
  ...
  brightValue.style.display = "block";
  brightValue.innerHTML = `${range.value}`;
...
document.getElementById("brightControl").onmouseover = () => {
  document.getElementById("brightValue").style.display = "block";
};

document.getElementById("brightControl").onmouseout = () => {
  document.getElementById("brightValue").style.display = "none";
};
...
/*
    ...
    let top1 = Math.floor(el.getBoundingClientRect().top);

    let top2 = Math.floor(el.getBoundingClientRect().top + parseFloat(height));

    let bottom1 = Math.floor(el.getBoundingClientRect().left);

    let bottom2 = Math.floor(
      el.getBoundingClientRect().left + parseFloat(width)
    );

let disp = getComputedStyle(el).getPropertyValue("display");

let param =

      event.clientX > bottom2 ||
      event.clientX < bottom1 ||
      event.clientY > top2 ||
      event.clientY < top1;
    
  showScreenCount += 1;
  
  if(showScreenCount > 1 && param) {
...
(() => {
  ...
  document.getElementById("vol-text").innerHTML = `${
    document.getElementById("vol").value
  }%`;

  let bigVal = parseFloat(vol.value);

  let smVal = parseFloat(100 - bigVal);

  vol.style.background =
    `linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
    `-o-linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
    `-ms-linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
    `-moz-linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
    `-webkit-linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)`;
})();
...
function volume() {
  ...
  volText.innerHTML = `${vol.value}%`;

  let bigVal = Number(vol.value);

  let smVal = Number(0 - bigVal);
...
document.getElementById("vol").onmouseover = () => {
  document.getElementById("vol-text").style.display = "block";
};

document.getElementById("vol").onmouseout = () => {
  document.getElementById("vol-text").style.display = "none";
};

let btns = Array.from(document.getElementsByClassName("player-btns"));

btns.forEach((value, index, array) => {
  value.addEventListener("click", () => {
    if (index === 0) {

      displayContent("screen-content");
    } else if (index === 1) {
...
function mute() {
  let mute = document.getElementById("mute-svg");

  let sound = document.getElementById("sound-svg");
...
// For the subtitle size

(() => {
  let sizeRange = document.getElementById("size-range");

  let bigVal = parseFloat(sizeRange.value);

  let smVal = Number(-20 + bigVal * 2);

  let sizeText = document.getElementById("size-text");

  sizeText.innerHTML = `${bigVal}`;

  sizeRange.style.background =
    `linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
    `-o-linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
    `-ms-linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
    `-moz-linear-gradient(to right, indigo ${bigVal}%, slategrey ${smVal}%)` ||
...
function subSize() {
  ...
  let bigVal = Number(sizeRange.value);
...
document.getElementById("size-range").onmouseover = () => {
  document.getElementById("size-text").style.display = "block";
};

document.getElementById("size-range").onmouseout = () => {
  document.getElementById("size-text").style.display = "none";
};

let subBtns = Array.from(document.getElementsByClassName("sub-btns"));

subBtns.forEach((value, index, array) => {
  value.addEventListener("click", () => {
    if (index === 0) {
      
      displayContent("sub-text-cont");
      
    } else if (index === 1) {
      
      displayContent("layout-content");
...
// For the subtitle checkbox

let elemSub = Array.from(document.getElementsByClassName("text-head"));
...
// For the player checkbox

let elemPlayer = Array.from(document.getElementsByClassName("control-inner-text"));

elemPlayer.forEach(el => {
...
// For the display Content closure

document.getElementById("screen-guard").onclick = (e) => {
    
   e.currentTarget.style.display = "none";
   
 Array.from(document.getElementsByClassName("player-items")).forEach(el => {
   el.style.display = "none"
 })
 
  Array.from(document.getElementsByClassName("sub-main-cont")).forEach(el => {
