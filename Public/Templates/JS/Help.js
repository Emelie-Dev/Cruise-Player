...
let firstArrow1 = arrow[2].innerHTML;

let firstArrow2 = arrow[1].innerHTML;

let firstArrow3 = arrow[0].innerHTML;

function report() {
  let doc = document.getElementById("report-content");

  const disp = getComputedStyle(doc).getPropertyValue("display");

  if (disp == "block") {
    doc.style.display = "none";
    arrow[2].innerHTML = firstArrow1;
  } else {
    arrow[2].innerHTML = secondArrow;
    doc.style.display = "block";
    document.getElementById("report-text").scrollIntoView();
   
  }
}

function feedback() {
  let doc = document.getElementById("feedback-content");

  const disp = getComputedStyle(doc).getPropertyValue("display");

  if (disp == "block") {
    doc.style.display = "none";
    arrow[1].innerHTML = firstArrow2;
  } else {
    arrow[1].innerHTML = secondArrow;
    doc.style.display = "block";
    document.getElementById("feed-text").scrollIntoView();
...
function features() {
  let doc = document.getElementById("features-content");

  const disp = getComputedStyle(doc).getPropertyValue("display");

  if (disp == "block") {
    doc.style.display = "none";
    arrow[0].innerHTML = firstArrow3;
  } else {
    arrow[0].innerHTML = secondArrow;
    doc.style.display = "block";
    document.getElementById("feature-text").scrollIntoView();
  }
}
...
function chance() {
  ...
  let val = "";
  document.getElementById("file-no" ).innerHTML = ``;


  if ("files" in file) {
    for (let i = 0; i < file.files.length; i++) {
      val += `<b style="color: indigo;text-shadow: 0px 0px 0px black;">${
        i + 1
      }. </b>`;
      let param = file.files[i];
      ...
      if(file.files.length < 1) {
       ...
       val = "";
        ...
        val = `You can only choose two files.<br>`;

        document.getElementById("file-no").innerHTML = "";
      }  else {
        if ("name" in param) {
          val += `<b>Filename:</b> ${param.name}<br>`;
   document.getElementById(
            ...
            ).innerHTML = `<b>No. of file(s):</b> ${file.files.length}`;
        }

        if ("size" in param) {
          let size = (param.size / 1024).toFixed(2);
          if (size > 1024) {
            val = `File size cannot be more than 1MB.`;

            document.getElementById("file-no").innerHTML = ``;
          } else {
            val += `&nbsp;&nbsp;&nbsp;&nbsp;<b>Filesize:</b> ${size} KB <br><br>`;
...
function reportValidate() {
  if (
    ...
    document.getElementById("report-submit").style.opacity = 1;
    document.getElementById("report-submit").style.pointerEvents = "all";
  } else {
    document.getElementById("report-submit").style.opacity = 0.5;
    document.getElementById("report-submit").style.pointerEvents = "none";
  }
}

// For the star rating

const d = document.getElementsByClassName("feedback-svg");

for (let i = 0; i < d.length; i++) {
  d[i].onclick = () => {
    if (d[i].style.fill !== "magenta") {
      for (let v = 0; v <= i; v++) {
        d[v].style.fill = "magenta";
      }
    } else {
      for (let x = i + 1; x >= i; x++) {
        d[x].style.fill = "transparent";
      }
    }
  };
}

// For the feedback validation

function feedbackValidate() {
  if (document.getElementById("feedback-text").value !== "") {
    document.getElementById("feedback-submit").style.opacity = 1;
    document.getElementById("feedback-submit").style.pointerEvents = "all";
  } else {
    document.getElementById("feedback-submit").style.opacity = 0.5;
    document.getElementById("feedback-submit").style.pointerEvents = "none";
  }
}
...
setInterval(() => {
  ...
  if (d[i].style.fill == "magenta" || d[0].style.fill == "magenta") {
    document.getElementById("feedback-submit").style.opacity = 1;
    document.getElementById("feedback-submit").style.pointerEvents = "all";
  } else {
    document.getElementById("feedback-submit").style.opacity = 0.5;
...
function rateValue() {
  if (d[4].style.fill === "magenta") {
    document.getElementById("rate-number").value = 5;
  } else if (d[3].style.fill === "magenta") {
    document.getElementById("rate-number").value = 4;
  } else if (d[2].style.fill === "magenta") {
    document.getElementById("rate-number").value = 3;
  } else if (d[1].style.fill === "magenta") {
    document.getElementById("rate-number").value = 2;
  } else if (d[0].style.fill === "magenta") {
    document.getElementById("rate-number").value = 1;
  }
}
